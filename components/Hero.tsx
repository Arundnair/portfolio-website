...
const Hero: React.FC = () => {
  ...
  // Map name words for rendering
  const nameWords = useMemo(() => PERSONAL_INFO.name.split(" "), []);
    ...
    >
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col items-center gap-8">
          <nav className="flex justify-center gap-6" aria-label="Social links">
            {SOCIALS.map((social, i) => (
              <a 
                ...
                className="group relative p-3 md:p-4 rounded-full bg-white/80 dark:bg-white/5 border border-black/5 dark:border-white/10 hover:border-black/20 dark:hover:border-white/30 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-neon-blue focus-visible:ring-offset-2"
              >
                <div className="absolute inset-0 bg-neon-blue/10 rounded-full scale-0 group-hover:scale-100 transition-transform duration-300" aria-hidden="true" />
                {React.cloneElement(SOCIAL_ICONS[social.icon] as React.ReactElement<any>, { 
                  size: 24, 
                  className: "relative z-10 text-gray-700 dark:text-gray-300 group-hover:text-black dark:group-hover:text-white transition-colors",
                  'aria-hidden': true
                })}
              </a>
            ))}
